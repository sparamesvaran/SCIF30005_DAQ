...
void core1_temperature_read() {
    ...
    while (samples_sent<events_to_send)
    {
        ...
        if (sample_buffer_index%BUFFER_CHUNK==0)
        {
...
int main() {
    ...
    // set up queue
    queue_init(&buffer_index_queue, sizeof(uint32_t), BUFFER_INDEX_QUEUE_SIZE);
    ...
    while (run_processing 
        && samples_processed<events_to_send
        )
    {
        uint32_t end_index;
        queue_remove_blocking(&buffer_index_queue, &end_index);

        uint32_t start_index = end_index - BUFFER_CHUNK;
